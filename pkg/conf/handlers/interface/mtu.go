...
func init() {
	handlers.RegisterFactory(NewMTUHandler)
}
...
func NewMTUHandler(daemons *handlers.ConfDeps) handlers.Handler {
	return &MTUHandler{dataplane: daemons.Dataplane}
}

func (h *MTUHandler) Validate(ctx context.Context, hctx *handlers.HandlerContext) error {
	var mtu int
...
func (h *MTUHandler) Apply(ctx context.Context, hctx *handlers.HandlerContext) error {
...
func (h *MTUHandler) Rollback(ctx context.Context, hctx *handlers.HandlerContext) error {
	ifName := handlers.ExtractInterfaceName(hctx.Path)
