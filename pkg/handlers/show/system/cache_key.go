...
func init() {
	show.RegisterFactory(NewCacheKeyHandler)
}
...
func NewCacheKeyHandler(deps *deps.ShowDeps) show.ShowHandler {
	return &CacheKeyHandler{
		cache: deps.Cache,
	}
}

func (h *CacheKeyHandler) Collect(ctx context.Context, req *show.Request) (interface{}, error) {
	key := req.Options["key"]
	if key == "" {
